package commerce.com.board.controller;
...
import commerce.com.board.service.BoardService;
import commerce.com.util.StringUtil;
...
public class BoardController {
  ...
  private BoardService boardService;
	...
	@Value("${system.inside.ip}")  // from global.yml
	private String insideIp;

	@Autowired
	private MessageSource messageSource;    // from message_**.yml
  ...
  public HashMap<String, Object> selectBoardList(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		log.debug(" ****  [BoardController  selectBoardList]  **********  "  );
		 // global.yml test
    	log.debug(":::: insideIp :  "+insideIp);
    	 // message.yml test
    	log.debug(":::: msg.error.pagenotfound Default :  "+messageSource.getMessage("msg.error.pagenotfound", null, Locale.getDefault()));
    	log.debug(":::: msg.error.pagenotfound US :  "+messageSource.getMessage("msg.error.pagenotfound", null, Locale.US));
    	log.debug(":::: msg.error.pagenotfound KOREA :  "+messageSource.getMessage("msg.error.pagenotfound", null, Locale.KOREA));



    String schType = StringUtil.returnObjectToString(request.getParameter("schType"));
    String schVal = StringUtil.returnObjectToString(request.getParameter("schVal"));
    int rows = Integer.parseInt(StringUtil.returnObjectToStringKeyValue(request.getParameter("rows"),"10"));
    int page = Integer.parseInt(StringUtil.returnObjectToStringKeyValue(request.getParameter("page"),"1"));
    String[] sort = URLDecoder.decode(StringUtil.returnObjectToStringKeyValue(request.getParameter("sort"),"DOC_NO desc"), "UTF-8").split(",");
    ...
    vo.setSort(sort);
	log.debug(" ****  [BoardController  selectBoardList]  vo : "+vo.toString()  );

    List<BoardVO> result = boardService.selectBoardList(vo);
	log.debug(" ****  [BoardController  selectBoardList]  result : "+ result );
